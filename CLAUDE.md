...
- Astro routes: middleware calls `next()` to let Astro render
- Everything else: proxies to Shopify (`www.southlandorganics.com`)
- HTMLRewriter replaces Shopify header/footer with our mega-menu partials
- Section IDs confirmed matching: `#shopify-section-header--default`, `#shopify-section-footer`, `#shopify-section-subfooter`
...
**HTMLRewriter + Partials:**
- `build-partials` renders `StaticHeader`/`Footer` to `public/_partials/`
- Partials loaded via ASSETS binding (`context.locals.runtime.env.ASSETS`)
- Verified working on `southland-website.pages.dev`

**DNS Cutover Blocker:**
- Current proxy target is `www.southlandorganics.com` (works because DNS points to Shopify)
- When DNS flips to Cloudflare Pages, this creates a loop
- Need a CNAME origin subdomain (e.g. `shopify-origin.southlandorganics.com → shops.myshopify.com`)
