...
export default function CDPDashboard() {
  ...
  const personaData = metrics?.personaDistribution.map(p => ({
    name: p.label,
    value: p.count,
  })) || [];

  const funnelData = metrics?.journeyFunnel.map(s => ({
    name: s.label,
    value: s.count,
  })) || [];
      ...
      <Grid numItemsMd={2} numItemsLg={4} className="gap-6">
        <Card decoration="top" decorationColor="amber">
          <Flex alignItems="start">
            <div>
              ...
              <Metric>{formatNumber(metrics?.personaDistribution.reduce((a, b) => a + b.count, 0) || 0)}</Metric>
            </div>
        ...
        <Card decoration="top" decorationColor="emerald">
          <Flex alignItems="start">
            <div>
              ...
              <Metric>{formatNumber((metrics?.tunnels.betty.conversions || 0) + (metrics?.tunnels.bill.conversions || 0) + (metrics?.tunnels.taylor.conversions || 0))}</Metric>
            </div>
        ...
        <Card decoration="top" decorationColor="blue">
          <Flex alignItems="start">
            <div>
              <Text>Revenue</Text>
              <Metric>{formatCurrency((metrics?.tunnels.betty.revenue || 0) + (metrics?.tunnels.bill.revenue || 0) + (metrics?.tunnels.taylor.revenue || 0))}</Metric>
            </div>
        ...
        <Card decoration="top" decorationColor="violet">
          <Flex alignItems="start">
            <div>
              ...
              <Metric>{((metrics?.tunnels.betty.conversionRate || 0) + (metrics?.tunnels.bill.conversionRate || 0) + (metrics?.tunnels.taylor.conversionRate || 0) / 3).toFixed(1)}%</Metric>
            </div>
      ...
      <Grid numItemsMd={2} className="gap-6">
        ...
        <Card>
          <Flex alignItems="start">
            ...
            <Badge color="emerald" size="sm">Winner: Tunnel</Badge>
          </Flex>
          ...
          <div className="mt-6 space-y-6">
            <div>
              ...
              <ProgressBar value={metrics?.abTests.tunnelVsGeneric.control.conversionRate || 0} color="gray" className="mt-2" />
            </div>
            <div>
              <Flex>
                <Text>Personalized Tunnel</Text>
                <Text className="font-semibold text-emerald-600">{metrics?.abTests.tunnelVsGeneric.treatment.conversionRate}%</Text>
              </Flex>
              <ProgressBar value={metrics?.abTests.tunnelVsGeneric.treatment.conversionRate || 0} color="emerald" className="mt-2" />
            </div>

            <Card className="bg-emerald-50 border-emerald-200">
              <Flex>
                <div>
                  <Text className="text-emerald-800 font-medium">Tunnel variant is winning</Text>
                  <Text className="text-emerald-600">+{metrics?.abTests.tunnelVsGeneric.treatment.lift?.toFixed(0)}% lift at {metrics?.abTests.tunnelVsGeneric.confidence}% confidence</Text>
                </div>
                <Metric className="text-emerald-700">{metrics?.abTests.tunnelVsGeneric.treatment.conversionRate}%</Metric>
              </Flex>
            </Card>
          </div>
        </Card>
      ...
      <Grid numItemsMd={3} className="gap-6">
        ...
        <Card className="bg-gradient-to-br from-amber-50 to-orange-50">
          <Flex alignItems="start">
            ...
            <BadgeDelta deltaType={metrics?.tunnels.betty.trend && metrics.tunnels.betty.trend >= 0 ? 'increase' : 'decrease'}>
          ...
          <Flex className="mt-4 space-x-4">
            <div className="flex-1">
              ...
              <Text className="font-semibold text-amber-900">{formatNumber(metrics?.tunnels.betty.views || 0)}</Text>
            </div>
            <div className="flex-1">
              <Text className="text-xs text-amber-700">Conv.</Text>
              <Text className="font-semibold text-amber-900">{formatNumber(metrics?.tunnels.betty.conversions || 0)}</Text>
            </div>
            <div className="flex-1">
              <Text className="text-xs text-amber-700">Revenue</Text>
              <Text className="font-semibold text-amber-900">{formatCurrency(metrics?.tunnels.betty.revenue || 0)}</Text>
            </div>
          </Flex>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-emerald-50 to-green-50">
          <Flex alignItems="start">
            ...
            <BadgeDelta deltaType={metrics?.tunnels.bill.trend && metrics.tunnels.bill.trend >= 0 ? 'increase' : 'decrease'}>
          ...
          <Flex className="mt-4 space-x-4">
            <div className="flex-1">
              ...
              <Text className="font-semibold text-emerald-900">{formatNumber(metrics?.tunnels.bill.views || 0)}</Text>
            </div>
            <div className="flex-1">
              <Text className="text-xs text-emerald-700">Conv.</Text>
              <Text className="font-semibold text-emerald-900">{formatNumber(metrics?.tunnels.bill.conversions || 0)}</Text>
            </div>
            <div className="flex-1">
              <Text className="text-xs text-emerald-700">Revenue</Text>
              <Text className="font-semibold text-emerald-900">{formatCurrency(metrics?.tunnels.bill.revenue || 0)}</Text>
            </div>
          </Flex>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-cyan-50 to-teal-50">
          <Flex alignItems="start">
            ...
            <BadgeDelta deltaType={metrics?.tunnels.taylor.trend && metrics.tunnels.taylor.trend >= 0 ? 'increase' : 'decrease'}>
          ...
          <Flex className="mt-4 space-x-4">
            <div className="flex-1">
              ...
              <Text className="font-semibold text-cyan-900">{formatNumber(metrics?.tunnels.taylor.views || 0)}</Text>
            </div>
            <div className="flex-1">
              <Text className="text-xs text-cyan-700">Conv.</Text>
              <Text className="font-semibold text-cyan-900">{formatNumber(metrics?.tunnels.taylor.conversions || 0)}</Text>
            </div>
            <div className="flex-1">
              <Text className="text-xs text-cyan-700">Revenue</Text>
              <Text className="font-semibold text-cyan-900">{formatCurrency(metrics?.tunnels.taylor.revenue || 0)}</Text>
            </div>
          </Flex>
        </Card>
      </Grid>
            ...
            {events.slice(0, 8).map((event) => (
              <TableRow key={event.id}>
                <TableCell>
                  <Badge
                    color={
                      event.type.includes('conversion') ? 'emerald' :
                      event.type.includes('tunnel') ? 'blue' :
                      event.type.includes('persona') ? 'amber' : 'gray'
                    }
                  >
