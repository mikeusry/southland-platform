...
export async function findRelatedContent(
  ...
  const { pageTypes = ['blog', 'page'], matchThreshold = 0.75, matchCount = 5 } = options
 ...
 */
export async function findTargetPersona(episodeEmbedding: number[]): Promise<Persona | null> {
  if (!supabase) {
...
export async function getPersonas(): Promise<Persona[]> {
  ...
  try {
    const { data, error } = await supabase.from('personas').select('*').eq('brand_id', 'southland')
