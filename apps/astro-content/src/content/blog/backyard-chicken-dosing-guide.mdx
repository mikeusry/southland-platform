...
- Keep at least 3 days apart from apple cider vinegar — Big Ole Bird and ACV are two different types of probiotics, and running them in the same water cancels out the beneficial microbes in both
...
Yes for water-soluble products, with one exception. Big Ole Bird, Hen Helper, and Catalyst can all go in the same waterer at the same time without conflict. The exception is Mother Load ACV: it's a different type of probiotic than Big Ole Bird and Hen Helper, so keep ACV at least 3 days apart from those two — running them together cancels out the beneficial microbes. (Catalyst is a vitamin, so it mixes with anything, including ACV.) Litter Life is applied to bedding, not water. Desecticide is a topical or coop spray, never in water. Refresh the waterer every 24-48 hours so the mix stays fresh.
