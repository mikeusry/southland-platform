...
| File | Purpose |
|------|---------|
| `src/components/bundle/CaseBuilder.tsx` | Main React island |
| `src/components/bundle/ProductSelector.tsx` | Product card with +/- controls |
...
- Shopify collection handle `gallon-products` (must not be renamed by ops)
...
1. Tag gallon products with `gallon-size`
2. Create automated collection "Gallon Products" with handle `gallon-products`
3. Create Automatic Discount: buy 4+ from collection, percentage off
